...
$(window).scroll(function() {    
	...
	if ($('body').hasClass('page-node-1')) {
		var companies_section = $('#world_leading_companies');
		var companies_offset = companies_section.offset();
		if (companies_section.hasClass("companies-animated") != "ture" && scroll >= companies_offset.top-height_header-200 && $('body').hasClass('page-node-1')) {
			companies_section.addClass("companies-animated");
		}
	}
});
...
$(document).ready(banner_padding);

function banner_padding() {
	if($('body').hasClass('page-node-1')){
		var title_content_height, title_padding;
		title_height = $('.title_section').height();
		title_content_height = $('.title_section .container').height();
		title_padding = (title_height - 650  - title_content_height)/2;
		title_top_padding = title_padding-50;
		title_bottom_padding = title_padding+68;
		title_top_slide_padding = title_padding-50;
		title_bottom_slide_padding = title_padding+68+20;
		if(title_top_padding <= 30){
			title_top_padding = 52;
			title_bottom_padding = 80;
			title_top_slide_padding = 62;
			title_bottom_slide_padding = 150;
		}
		...
		$('.animation_slides .container').show();
		
		$('.animation_slides .slick-dots').css('top', title_content_height+title_top_padding+128);
	} else { 
		var title_content_height, title_padding;
		title_content_height = $('.title_section .container').height();
		title_padding = (300 - 64 - title_content_height)/2;
...
/* ********* Jobs Ajax ********* */
$('#jobsorting a').click(function(e) {
	e.preventDefault();
	$(this).addClass('active');
	$(this).siblings().removeClass('active');
	var sortkey = $(this).data('sort'), department = $(this).data('department'), get_gh_src = $(this).data('ghsrc');
	if(get_gh_src === ''){ get_gh_src = 'null'; }
	var	url = '/api/jobs_ajax/'+department+'/'+sortkey+'/'+get_gh_src; 
	$.get(url).done(function( result ) {
...
if($('.title_section').length){
	if ($('.title_section').siblings('.heronav_section').length) {
			$('.title_section').addClass('with_heronav');
	} else {
				$('.title_section').addClass('without_heronav');
	}
}	
})(jQuery);
/* ===== captiansContainer ===== */
...
$(document).ready(function() {
  $(window).on('load', function() {
    $('#search-form .search-advanced .form-item-type label[for="edit-type"]').html('Only search the following:');
  });
  $('#search_box_icon').on('click touch', function(e) { 
    e.preventDefault();
    $('#search-block-form').addClass('active');
    $('.search_box_icon').addClass('inactive');
    $('.nav-user').addClass('inactive');
    $('#search-block-form .container-inline input[type="text"]').focus();
  });
  $('#search-block-form #edit-close').on('click', function(e) {
    e.preventDefault();
    $('#search-block-form').removeClass('active');
    $('.search_box_icon').removeClass('inactive');
    $('.nav-user').removeClass('inactive');
  });
  $('#search-block-form input.form-text').prop('required',true);

  var submitSearchAjax_timer;

  $(document).on('click', '#pager .pager li a', function(e) {
    e.preventDefault();
    submitSearchLoading();
    clearTimeout(submitSearchAjax_timer);
    submitSearchAjax_timer = setTimeout(submitSearchAjax2($(this)), 400);
  });
  $(document).on('submit', '#search-form', function(e) {
    e.preventDefault();
    submitSearchLoading();
    clearTimeout(submitSearchAjax_timer);
    submitSearchAjax_timer = setTimeout(submitSearchAjax, 400);
  });
  $(document).on('keyup', '#edit-keys', function(e) {
    e.preventDefault();
    submitSearchLoading();
    clearTimeout(submitSearchAjax_timer);
    submitSearchAjax_timer = setTimeout(submitSearchAjax, 400);
  });
  $(document).on('change', '#search-form .form-checkbox', function(e) {
    e.preventDefault();
    submitSearchLoading();
    clearTimeout(submitSearchAjax_timer);
    submitSearchAjax_timer = setTimeout(submitSearchAjax, 400);
  });
  var submitSearchLoading = function() {
    var $devPosts = $('#docker-search #results');
    if($devPosts.length) {
      if(!$devPosts.hasClass('searching')) {
        $('#docker-search #results').addClass('searching');
      }
    }
  }
  var submitSearchAjax = function() {
    var $devSearchtotals = $('#docker-search #search-totals');
    var $devPosts = $('#docker-search #results');
    var $devPagination = $('#docker-search #pager');
    var $searchForm = $('#search-form');
    var $searchFormUrl = $searchForm.attr('action');
    ...
    $.ajax({
      ...
      success: function(response) {
        $searchtotals = $(response).find('#docker-search #search-totals').html();
        $posts = $(response).find('#docker-search #results').html();
        $pagination = $(response).find('#docker-search #pager').html();
        if($devSearchtotals.length) {
          if(typeof $searchtotals === 'undefined') { $searchtotals = ''; }
          $('#docker-search #search-totals').html($searchtotals);
        } else {
          if(typeof $searchtotals === 'undefined') { $searchtotals = ''; }
          $('#docker-search #block-system-main').append('<div class="search-totals" id="search-totals">'+$searchtotals+'</div>');
        }
        if($devPosts.length) {
          if(typeof $posts === 'undefined') { $posts = '<h2>Your search yielded no results</h2><ul><li>Check if your spelling is correct.</li><li>Remove quotes around phrases to search for each word individually. <em>bike shed</em> will often show more results than <em>"bike shed"</em>.</li><li>Consider loosening your query with <em>OR</em>. <em>bike OR shed</em> will often show more results than <em>bike shed</em>.</li></ul>'; }
          $('#docker-search #results').html($posts).removeClass('searching');
        } else {
          if(typeof $posts === 'undefined') { $posts = '<h2>Your search yielded no results</h2><ul><li>Check if your spelling is correct.</li><li>Remove quotes around phrases to search for each word individually. <em>bike shed</em> will often show more results than <em>"bike shed"</em>.</li><li>Consider loosening your query with <em>OR</em>. <em>bike OR shed</em> will often show more results than <em>bike shed</em>.</li></ul>'; }
          $('#docker-search #block-system-main').append('<div id="results">'+$posts+'</div>');
        }
        if($devPagination.length) {
          if(typeof $pagination === 'undefined') { $pagination = ''; }
          $('#docker-search #pager').html($pagination);
        } else {
          if(typeof $pagination === 'undefined') { $pagination = ''; }
          $('#docker-search #block-system-main').append('<div id="pager">'+$pagination+'</div>');
        }
      }
    });
  };
  var submitSearchAjax2 = function(href) {
    var $devSearchtotals = $('#docker-search #search-totals');
    var $devPosts = $('#docker-search #results');
    var $devPagination = $('#docker-search #pager');
    var $searchUrl = href.attr('href');
    $.ajax({
      url: $searchUrl,
      type: 'GET',
      dataType: 'html',
